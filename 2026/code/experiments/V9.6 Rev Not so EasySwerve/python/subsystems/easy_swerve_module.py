...
from constants import easy_swerve_module_config, DIORearLeftEncoderConstants, DIORrontLeftEncoderConstants
...
class EasySwerveModule:
	...
	def __init__(self, driving_can_id: int, turning_can_id: int, chassis_angular_offset: float,
		...
		# DIO-based encoder support (for rear left motor)
		self.use_dio_encoder = False
		self.dio_encoder = None
		self.last_encoder_position = None
		self._last_continuous_angle = None  # Track continuous angle (not wrapped)
		self.encoder_debounce_count = 0
		self.encoder_debounce_threshold = 5  # Require 5 stable reads
		self.teleop_enabled = False
		
		# Skip periodic command during manual zeroing (allows direct motor.set())
		self.skip_periodic_command = False
		
		# Initialize DIO encoder if this is rear left or front left module
		if module_name == "Rear Left":
			try:
				print(f"[{module_name}] Attempting to initialize quadrature encoder...")
				# Create encoder: A on DIO0, B on DIO1
				self.dio_encoder = wpilib.Encoder(
					DIORearLeftEncoderConstants.k_rear_left_dio_channel_a,
					DIORearLeftEncoderConstants.k_rear_left_dio_channel_b
				)
				
				# Configure for REV Through-Bore Encoder V2: 352 CPR based on measured data
				# Measured: 44 counts = 45 degrees (pi/4 rad), so distance_per_pulse = (pi/4)/44 = pi/176
				self.dio_encoder.setMaxPeriod(0.1)
				self.dio_encoder.setSamplesToAverage(2)
				self.dio_encoder.setDistancePerPulse(math.pi / 176.0)  # pi/176 rad per count
				self.dio_encoder.reset()
				
				self.use_dio_encoder = True
				print(f"[{module_name}] SUCCESS: Quadrature encoder initialized on DIO{DIORearLeftEncoderConstants.k_rear_left_dio_channel_a}/{DIORearLeftEncoderConstants.k_rear_left_dio_channel_b}")
			except Exception as e:
				print(f"[{module_name}] FAILURE: Failed to initialize quadrature encoder: {e}")
				import traceback
				traceback.print_exc()
				self.use_dio_encoder = False
		elif module_name == "Front Left":
			try:
				print(f"[{module_name}] Attempting to initialize quadrature encoder...")
				# Create encoder: A on DIO3, B on DIO4
				self.dio_encoder = wpilib.Encoder(
					DIORrontLeftEncoderConstants.k_front_left_dio_channel_a,
					DIORrontLeftEncoderConstants.k_front_left_dio_channel_b
				)
				...
				self.use_dio_encoder = True
				print(f"[{module_name}] SUCCESS: Quadrature encoder initialized on DIO{DIORrontLeftEncoderConstants.k_front_left_dio_channel_a}/{DIORrontLeftEncoderConstants.k_front_left_dio_channel_b}")
			except Exception as e:
				print(f"[{module_name}] FAILURE: Failed to initialize quadrature encoder: {e}")
				import traceback
				traceback.print_exc()
			...
			print(f"[{module_name}] Not Rear Left or Front Left, skipping DIO encoder init")
	...
	def read_dio_encoder(self, skip_debounce: bool = False) -> float:
		"""
		Read absolute encoder position from quadrature A/B channels.
		Counts state transitions to determine angle in radians.
		
		Args:
			skip_debounce: If True, return value immediately without debouncing (for testing)
		
		Returns:
			Angle in radians (0 to 2π), or None if DIO encoder not available
		"""
		if not self.use_dio_encoder or self.dio_encoder is None:
			return None
		
		try:
			# Get raw count value (before distance conversion)
			raw_count = self.dio_encoder.getRaw()
			# Get the current distance in radians (already converted from counts)
			# This is a CONTINUOUS value that doesn't wrap - it keeps counting up as you rotate
			continuous_angle = self.dio_encoder.getDistance()
			
			# REVERSE the encoder to match motor direction
			continuous_angle = -continuous_angle
			
			# For display and debounce, track continuous angle first
			if skip_debounce:
				# Return normalized angle for display: 0 to 2π
				normalized = continuous_angle % (2.0 * math.pi)
				if normalized < 0:
					normalized += 2.0 * math.pi
				return normalized
			
			# DEBOUNCE BASED ON CONTINUOUS ANGLE (no wraparound)
			# This avoids the 0/2π jump problem
			if self._last_continuous_angle is not None and abs(continuous_angle - self._last_continuous_angle) < 0.01:
				self.encoder_debounce_count += 1
			else:
				self.encoder_debounce_count = 0
			
			self._last_continuous_angle = continuous_angle
			
			# Only return after debounce threshold
			if self.encoder_debounce_count >= self.encoder_debounce_threshold:
				# Return normalized angle for display: 0 to 2π
				normalized = continuous_angle % (2.0 * math.pi)
				if normalized < 0:
					normalized += 2.0 * math.pi
			...
			print(f"[{self.module_name}] ERROR reading quadrature encoder: {e}")
			return None
	...
	def periodic_turn_command(self):
		...
		"""
		# Skip if manually controlling (during zeroing)
		if self.skip_periodic_command:
			return
		
		# Read and print DIO encoder if enabled and teleop is active
		if self.use_dio_encoder and self.teleop_enabled:
			dio_angle = self.read_dio_encoder()
			if dio_angle is not None:
				# Save as latest stable value
				self.last_dio_value = dio_angle
				# Print radians (control value) - raw bit display happens in read_dio_encoder
				print(f"[{self.module_name}] DIO: {dio_angle:7.4f} rad")
