			...
			while True:
				try:
					...
					if cmd == "focus":
						...
						max_speed = value.get("max_speed", 0.8)
						...
						dashboard.table.putNumber("navigation_max_speed", max_speed)
						dashboard.table.putBoolean("navigate_waypoints_command", True)
						print(f"[WS] Waypoint route: {len(waypoints)} points loop={loop} spline={use_spline} speed={max_speed}")
						ws.send(json.dumps({"type": "success", "message": f"Route started: {len(waypoints)} waypoints (spline={use_spline}) speed={max_speed:.2f}"}))
