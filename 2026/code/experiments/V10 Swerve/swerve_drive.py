...
class SwerveDrive:
	...
	def drive_swerve(self, forward, strafe, rotate):
		...
		# Debug: print calculated angles once per second
		if not hasattr(self, 'last_swerve_debug'):
			self.last_swerve_debug = 0
		if time.time() - self.last_swerve_debug > 1.0:
			print(f"[SWERVE] Input: F={forward:.2f} S={strafe:.2f} R={rotate:.2f}")
			for wname, wdata in wheel_vectors.items():
				print(f"  {wname}: angle={wdata['angle']:.1f}° speed={wdata['speed']:.2f}")
			self.last_swerve_debug = time.time()
		...
		for wheel_name, wheel_data in wheel_vectors.items():
			...
			# Optimize: if wheel needs to rotate >90°, flip angle and reverse speed instead
			...
			# If rotation needed is >90°, flip to opposite direction
			if abs(angle_diff) > 90:
				target_angle = (target_angle + 180) % 360
				target_speed = -target_speed
			
			# Set drive motor
			wheel.set_drive_power(target_speed * config.MOTOR_SCALE_TELEOP)
			
			# Only use per-wheel alignment if global alignment is NOT active
			if not self.aligning:
				# Align wheel to optimized target angle
				self.drive_wheel_to_angle(wheel_name, target_angle)
		
		# Update per-wheel alignment only if global alignment is NOT active
		if not self.aligning:
	...
	def update_single_wheel_alignment(self):
		...
		for wheel_name, align_info in self.wheel_alignment_state.items():
			...
			# Keep alignment active to maintain angle - PID will output small corrections
			if abs(error) < 2.0:
				# Still apply PID to maintain angle, don't stop or remove from tracking
				pass
