...
class Robot(wpilib.TimedRobot):
	...
	def testPeriodic(self):
		...
		# Apply joystick controls to the focused wheel or use swerve teleop
		if active_wheel:
			...
			if self.joystick.getRawButtonPressed(6):
				self.drive.set_wheel_zero(active_wheel)
				print(f"[ZEROING] {active_wheel} saved as 0°\n")
			
			# LB button - save as 90 degrees
			if self.joystick.getRawButtonPressed(5):
				self.drive.set_wheel_angle(active_wheel, 90)
				print(f"[ZEROING] {active_wheel} saved as 90°\n")
			
			# BACK button - save as 180 degrees
			if self.joystick.getRawButtonPressed(7):
				self.drive.set_wheel_angle(active_wheel, 180)
				print(f"[ZEROING] {active_wheel} saved as 180°\n")
		else:
			# TELEOP MODE: Swerve drive control
			# Left joystick: forward/strafe
			# Right joystick X-axis: rotation
			strafe = self.joystick.getRawAxis(0)  # Left thumb left/right
	...
	def teleopPeriodic(self):
		...
		# Get joystick inputs
		left_y = -self.joystick.getRawAxis(1)  # Forward/backward (negative because up is positive)
		left_x = -self.joystick.getRawAxis(0)  # Strafe left/right (negate for correct direction)
		right_x = self.joystick.getRawAxis(4)  # Rotation left/right (RIGHT STICK LEFT/RIGHT)
		
		# Apply joystick deadband
		DEADBAND = 0.1
		if abs(left_y) < DEADBAND:
			left_y = 0
		if abs(left_x) < DEADBAND:
			left_x = 0
		if abs(right_x) < DEADBAND:
			right_x = 0
		
		# Check if we have input
		has_input = abs(left_y) > 0 or abs(left_x) > 0 or abs(right_x) > 0
		
		if has_input:
			# Apply swerve kinematics
			self.drive.drive_swerve(left_y, left_x, right_x)
			self.had_teleop_input = True
		else:
			# Joystick released - center wheels to 0°
			if self.had_teleop_input:
				print("[TELEOP] Joystick released - centering wheels to 0°")
				# Stop drive motors, but keep turn motors running to maintain angle
				for wheel_name in self.drive.wheels.keys():
					self.drive.set_wheel_drive_power(wheel_name, 0)
					self.drive.drive_wheel_to_angle(wheel_name, 0)
				self.had_teleop_input = False
