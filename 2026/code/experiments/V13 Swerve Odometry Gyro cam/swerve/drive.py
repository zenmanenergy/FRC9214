...
class SwerveDrive:
	...
	def update_vision_pose(self, vision_pose: Pose2d, vision_confidence: float = 0.95):
		"""
		Update odometry with vision-based pose estimate.
		Useful for AprilTag or other vision-based localization.
		
		Args:
			vision_pose: Pose from vision system (AprilTags, etc.)
			vision_confidence: Confidence in vision estimate (0-1)
		"""
		self.odometry.update_vision_pose(vision_pose, vision_confidence)
		self.robot_angle = self.odometry.getPose().rotation()

	def set_vision_trust(self, trust_factor: float):
		"""
		Set how much to trust vision updates vs odometry.
		
		Args:
			trust_factor: 0-1, higher values trust vision more (default 0.8)
		"""
		self.odometry.set_vision_trust(trust_factor)
