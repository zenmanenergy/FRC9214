"""NavX2 MXP IMU wrapper for swerve drive with sensor fusion support.
...
The complementary filter blends IMU absolute yaw (95%) with wheel kinematics
heading (5%) to maintain accuracy while preventing glitches.

Example:
    imu = SwerveIMU(invert=False)
    while not imu.is_ready():
        time.sleep(0.1)
    imu.zero_heading()  # Point robot at field 0°
    
    # In main loop:
    odometry.update()
    imu.fuse_heading(odometry)  # Blend IMU into odometry heading
    print(f"Heading: {odometry.get_heading()}°")
"""
...
def is_calibrating(self) -> bool:
	...
	def is_connected(self) -> bool:
		"""True if IMU is connected and communicating."""
		return self.ahrs.isConnected()
	
	def is_ready(self) -> bool:
		"""True if IMU is fully calibrated and ready for use."""
...
def get_pitch(self) -> float:
	...
	def fuse_heading(self, odometry: 'SwerveOdometry') -> None:
		"""
	...
	def publish_dashboard(self) -> None:
		"""Publish IMU status and angles to SmartDashboard."""
