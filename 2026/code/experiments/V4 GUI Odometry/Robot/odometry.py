...
class Odometry:
	...
	def update(self):
		...
		# Update position using average of previous and new heading
		# (more accurate for large time steps)
		new_theta = self.theta - (delta_theta / 2.0)
		self.x += avg_distance * math.cos(new_theta)
		self.y += avg_distance * math.sin(new_theta)
