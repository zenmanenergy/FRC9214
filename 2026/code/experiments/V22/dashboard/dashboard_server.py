			...
			while True:
				try:
					...
					if cmd == "focus":
						...
						print(f"[WS] Waypoint route: {len(waypoints)} points loop={loop} speed={max_speed}")
						ws.send(json.dumps({"type": "success", "message": f"Route started: {len(waypoints)} waypoints speed={max_speed:.2f}"}))
