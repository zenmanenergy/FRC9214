...
class WaypointNavigator:
	"""
	Navigate robot through waypoints, driving a straight line to each in turn.
	Heading is blended gradually from the heading captured at the start of each
	leg to the target waypoint's heading, so rotation and translation finish
	together as a single combined movement.
	- Stage 0: Idle
	- Stage 1: Drive + rotate simultaneously toward the current target waypoint
	- Stage 2: Dwell at the waypoint, then advance (or loop/finish)
	"""
	...
	def __init__(self, swerve_drive):
		...
		# Per-leg heading blend state: captured at the start of each leg so
		# heading can be gradually blended from the actual current heading
		# toward the target waypoint's heading as translation progresses
		self.leg_start_x = 0.0
		self.leg_start_y = 0.0
		self.leg_start_heading = 0.0
		self.leg_total_distance = 0.0
	...
	def _begin_leg(self):
		...
		self.leg_start_x = self.drive.odometry.get_x()
		self.leg_start_y = self.drive.odometry.get_y()
		self.leg_start_heading = self.drive.odometry.get_heading()
		target = self.waypoints[self.current_waypoint_index]
		self.leg_total_distance = math.sqrt((target["x"] - self.leg_start_x)**2 + (target["y"] - self.leg_start_y)**2)
	...
	def update(self):
		...
		# Gradually blend from the heading captured at leg start toward the target
		# heading as translation progresses, so rotation and translation finish together
		if self.leg_total_distance > 1e-6:
			progress = max(0.0, min(1.0, 1.0 - (distance_to_target / self.leg_total_distance)))
		else:
			progress = 1.0
		desired_heading = lerp_angle(self.leg_start_heading, target_angle, progress)
		angle_diff = shortest_angle_diff(current_heading, desired_heading)
