...
class Arm:
	...
	def __init__(self, wpilib, ArmJoystick, elevator_motor, shoulder_motor, wrist_motor, grabber_motor):
		...
		self.elevatorUpSpeedFactor = 0.3
		self.elevatorDownSpeedFactor = 0.3
		...
		self.wristDownSpeedFactor = 0.15
		self.wristPresetFactor = 0.1
		self.wristBreakSpeed = 0.045
		self.wrist_min_angle = -160
		self.wrist_max_angle = 70
		...
		self.presets = {
			...
			"Y":    {"elevator": 104, "shoulder": 39, "wrist": 28}, #scoring on L4
			...
			"EDown": {"elevator": 30, "shoulder": 38, "wrist": 28},
			"EDownDown": {"elevator": 30, "shoulder": 10, "wrist": 28},

		}

		# Elevator PID Controller
		self.elevator_kP = 0.052 # Proportional gain (increase if it's too slow)
		...
		self.shoulder_kP = 0.025
		  # Adjust this for speed
		self.shoulder_kI = 0.002  # Only increase if it struggles to reach target
		self.shoulder_kD = 0.002  # Helps smooth stopping

		self.shoulder_pid = PIDController(
			self.shoulder_kP, self.shoulder_kI, self.shoulder_kD
		)
		self.shoulder_pid.setTolerance(0.5)  # Stops adjustments within 1 degree

		# Wrist PID Controller
		self.wrist_kP = 0.020  # Adjust this for speed
		self.wrist_kI = 0.002  # Only increase if it struggles to reach target
		self.wrist_kD = 0.002  # Helps smooth stopping

		self.wrist_pid = PIDController(
			self.wrist_kP, self.wrist_kI, self.wrist_kD
		)
		self.wrist_pid.setTolerance(0.5)  # Stops adjustments within 1.5 degrees
	...
	def move_to_preset(self):
		...
		elevator_speed = self.elevator_pid.calculate(self.real_elevator_position, target_elevator) * (self.elevatorUpSpeedFactor + .23)
		shoulder_speed = self.shoulder_pid.calculate(self.real_arm_angle, target_shoulder) * (self.shoulderUpSpeedFactor + .23)
		wrist_speed = -self.wrist_pid.calculate(self.real_wrist_angle, target_wrist) * (self.wristUpSpeedFactor + .2) 
		...
		if self.elevator_pid.atSetpoint():
			elevator_speed = 0
			# print("elevator at max")
		if self.elevator_pid.atSetpoint():
			elevator_speed = 0
		if self.shoulder_pid.atSetpoint():
			# print("shoulder at max")
			shoulder_speed = 0
		...
		if not self.limit_wrist(wrist_speed):  # Prevent movement beyond limits
			print("hit wrist limit")
			wrist_speed = self.wristBreakSpeed
		if not self.limit_shoulder(shoulder_speed):  # Prevent movement beyond limits
			print("hit shoulder limit")
	...
	def control_elevator(self, elevator_speed):
		...
		if self.real_elevator_position < 52:
			breakspeed=0
		else:
	...
	def control_shoulder(self, shoulder_speed):
		...
		if self.real_arm_angle < -70 and self.real_wrist_angle < -45:
			breakSpeed = 0
		...
		if abs(shoulder_speed) < self.maxShoulderBreakSpeed:  # Apply braking when joystick is neutral
			shoulder_speed = breakSpeed
	...
	def limit_wrist(self, wrist_speed):
		...
		# print("wrist_ground_angle", wrist_ground_angle,self.real_wrist_angle, self.real_arm_angle)

		# Prevent exceeding 90° relative to the ground
		if wrist_ground_angle >= self.wrist_max_angle and wrist_speed > 0:
			# print("prevent excess 90")
			return False  # Stop upward movement
